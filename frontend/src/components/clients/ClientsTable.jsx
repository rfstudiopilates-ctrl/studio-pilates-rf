...
import { getPlanExpiryInfo } from '../../lib/planExpiryWhatsApp';
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-white shadow-[0_8px_30px_rgba(26,26,26,0.04)]">
      <div className="overflow-x-auto">
        <table className="min-w-[980px] w-full border-collapse text-left md:min-w-full">
          <thead>
          ...
          <tbody className="divide-y divide-border/70">
            {clients.length === 0 ? (
              ...
              clients.map((client, index) => {
                const expiry = getPlanExpiryInfo(client.activePlanEndDate);
                  ...
                  >
                    ...
                    <td className="px-3 py-3 sm:px-4 md:px-5 md:py-4">
                      {client.activePlanName ? (
                        <div className="min-w-0 space-y-1">
                          <span className="inline-flex max-w-[10rem] truncate rounded-full border border-emerald-100 bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-success sm:max-w-[14rem] sm:px-2.5 sm:py-1 sm:text-xs">
                            {client.activePlanName}
                          </span>
                          {client.activePlanEndDate ? (
                            ...
                            >
                              {expiry?.isExpired ? 'Venció' : 'Vence'}{' '}
                              {formatDateDisplay(client.activePlanEndDate)}
                              {expiry ? ` · ${expiry.daysText}` : ''}
                            </p>
                          ) : null}
                        </div>
                      ) : (
                    ...
                    <td className="px-3 py-3 sm:px-4 md:px-5 md:py-4">
                      <div className="flex flex-nowrap items-center justify-end gap-1 sm:gap-1.5 md:gap-2 max-sm:flex-col max-sm:items-stretch">
                        {client.activePlanName && client.activePlanEndDate ? (
                          <WhatsAppPlanExpiryButton
                            variant="compact"
                            clientName={client.fullName}
                            clientPhone={client.phone}
                            planName={client.activePlanName}
                            endDate={client.activePlanEndDate}
                          />
                        ) : null}
                        <button
                          type="button"
