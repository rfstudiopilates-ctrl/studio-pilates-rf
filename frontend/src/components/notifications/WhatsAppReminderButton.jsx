...
import { openClassReminderWhatsApp } from '../../lib/classReminderWhatsApp';
  ...
  if (compact) {
    return (
      <div className={`inline-flex flex-col items-end ${className}`}>
        <button
          type="button"
          title={hasPhone ? 'Enviar recordatorio por WhatsApp' : 'Sin teléfono'}
          aria-label={`Recordatorio WhatsApp a ${clientName || 'alumno'}`}
          onClick={handleClick}
          disabled={!hasPhone || isLoading}
          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 text-xs font-medium text-emerald-800 transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-40"
        >
          <WhatsAppIcon className="h-3.5 w-3.5" />
          Recordatorio
        </button>
        {localError ? (
          <p className="mt-1 max-w-40 text-right text-[11px] leading-tight text-danger">
            {localError}
          </p>
        ) : null}
      </div>
  ...
  return (
    <div className={className}>
      <button
        ...
        onClick={handleClick}
        disabled={!hasPhone || isLoading}
        className="inline-flex h-9 items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-sm font-medium text-emerald-800 transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-40"
      >
        <WhatsAppIcon />
        Recordatorio
      </button>
